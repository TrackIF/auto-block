...
function runBlock (config, done) {
  ...
  config.errorSuppressWhen = config.errorSuppressWhen || {'data.retry': false}
  if (_.isString(config.errorSuppressWhen)) {
    config.errorSuppressWhen = [config.errorSuppressWhen]
  }
...
function finish (error, results, config) {
  ...
  if (done) {
    var suppress = validateWhen(error, config.errorSuppressWhen)
    if (suppress) {
      error = null
    }
