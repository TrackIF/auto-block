...
function lambdaHandler (handler) {
  // TODO   logging.lambdaStart(handler.event, handler.context)
  ...
  try {
    ...
    // TODO logging.log('Exception in auto block', e)
    autoCb(e)
  }
}
...
function lambdaFinish (error, results, handler) {
  ...
  if (error) {
    // TODO logging.lambdaFailure(error, handler.context)
  } else {
    // TODO logging.lambdaSuccess(data.response, handler.context)
  }

  // If no explicit done() provided, use the one from the lambda context
  var done = handler.done || handler.context.done
  if (done) {
...
function lambdaStep (results, callback, step, handler) {
  ...
  // TODO logging.lambdaStartStep(step.name, handler.context)
  ...
  var cb = (err, result) => {
    ...
    // }
    // TODO logging.lambdaFinishStep(step.name, handler.context, JSON.stringify(stepOutput, null, 2))
  ...
  try {
    ...
    // TODO logging.log(`Exception in step '${step.name}'`, e)
    cb(e)
  }
}
