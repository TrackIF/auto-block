...
function finish (error, results, config) {
  ...
  if (error) {
    config.failure(error, data)
  } else {
    config.success(data.response, data)
  }
...
function runStepFunc (results, callback, step, config) {
  ...
  config.startStep(step.name, config.data)
  ...
  var cb = (err, result) => {
    ...
    config.finishStep(step.name, config.data, JSON.stringify(stepOutput, null, 2))
