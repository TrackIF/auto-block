...
function runBlock (config, done) {
  ...
  config.onStart = buildLoggingHook(config.onStart)
  config.onSuccess = buildLoggingHook(config.onSuccess)
  config.onFailure = buildLoggingHook(config.onFailure)
  config.onStartStep = buildLoggingHook(config.onStartStep)
  config.onFinishStep = buildLoggingHook(config.onFinishStep)
...
function buildLoggingHook (step) {
  var func = () => {
  }
  var hook = step
