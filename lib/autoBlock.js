...
function buildBlock (block, config) {
  var autoBlock = _.transform(block, (asyncBlock, step, key) => {
    ...
    whenDeps = _.map(whenDeps, (value, key) => {
...
function runStepWhen (results, when) {
  // Convert singular .when to array
  var checks = when || []
  if (!_.isArray(checks)) {
    checks = [checks]
  }

  // For each check in .when...
  return _.every(checks, (value) => {
    var paths = value

    // Convert string notation to array so we can use _.every
    if (!_.isObject(paths)) {
      paths = [paths]
    }

    return _.every(paths, (expected, key) => {
      var path = key
      var inverted = false
      var truthy = false
...
function validateSettings (controller) {
  ...
  _.forOwn(controller.block, (value, key) => {
    validateBlockSettings(value, key)
  })
}
